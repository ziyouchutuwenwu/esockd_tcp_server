...
-export([on_client_connected/4, on_client_data/4, on_disconnected/2]).

on_client_connected(Transport, Socket, IP, Port) ->
  io:format("客户端 ~ts:~p 连接, 进程id ~p~n", [IP, Port, self()]),
  tcp_server_demo:send_by_socket(Transport, Socket),
  noreplay.

on_client_data(_Transport, _Socket, Cmd, InfoBin) ->
